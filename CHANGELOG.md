...
### What Changed
- **Dropped the dead `kiro-waylands` meta-package from the repo.** It was a
  July build (`26.07-01`) describing itself as a shell-config meta-package
  (bash/zsh/fish), with no build recipe left anywhere in the build trees and no
  package in the repo depending on it. It was the only repo package that was
...
- Removed the package and its detached signature, then re-ran `repo.sh` rather
  ...
  db from the full package glob, so a manual `repo-remove` would be undone on
  the next run; it also needs a `.db.tar.gz` name, while the published db is a
  bare `nemesis_repo.db`.
- Before deleting, the db was extracted and grepped unanchored for
  `kiro-waylands` so that a versioned `%DEPENDS%` (`kiro-waylands>=26.07`) or an
  `%OPTDEPENDS%` (`name: reason`) could not hide from an anchored match. The
  only hits were the three inside its own `desc` entry (`%NAME%`, `%BASE%`,
  `%PROVIDES%`) plus its `%FILENAME%`.
- All signatures were current before the run, so the signing loop in `repo.sh`
  was a no-op and no gpg passphrase prompt was hit.
- Verified after the rebuild: 484 package files, 484 db entries, no
  `kiro-waylands` in the db, zero orphaned `.sig` files. The 484/484 match
  matters because `repo-add -R` prunes superseded package files from disk — a
  lower count would have meant it pruned something unintended.
- `index.html` counts are regenerated by `repo.sh` from the freshly built db, so
